...
pub async fn run_pipeline(
    reranker_model: &TextRerank,
    input: ScoringInput,
) -> Result<ScoreResult> {
    let (tfidf_earned, keyword_match_rate) = tfidf::tfidf_score(&input.resume_sections, &input.job_sections);
    
    let pairs = biencoder::biencoder_pairs(&input.resume_vectors, &input.job_vectors);
    
    let ranked = reranker::rerank(reranker_model, pairs)?;
